...
struct MaxBlockDepthPass : public PassWrapper<MaxBlockDepthPass, OperationPass<func::FuncOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(MaxBlockDepthPass)

  StringRef getArgument() const final { return "max-block-depth"; }
  StringRef getDescription() const final { return "Calculates max block depth and attaches it to func."; }
    ...
    for (Block &block : region.getBlocks()) {
      for (Operation &op : block) {
        for (Region &nestedRegion : op.getRegions()) {
          maxD = std::max(maxD, calculateMaxDepth(nestedRegion, currentDepth + 1));
        }
      }
    }
    return maxD;
